	...
	<div class="file-layout">
		...
		<el-dialog v-model="UploadDialog" title="上传文件" width="50%" center destroy-on-close>
			<el-upload
				...
				:action="localServer + '/file/upload'"
				...
				<el-collapse-transition>
					<div v-show="refresh">
						<el-checkbox-group v-model="checkList">
							...
							>
								<el-checkbox :label="index" style="position: absolute; left: 2px; top: -7px"
									><br
								/></el-checkbox>
								<div @click="open(item.filepath, item.filename)">
									<img
										v-if="!item.filename.toLowerCase().endsWith('.mp4')"
										:src="selectCover(item.filename.toString(), item.filepath.toString())"
										class="image"
									/>
									<video
										v-else
										:src="videosrc(item.filepath)"
										class="image"
										preload="auto"
								...
								<div class="elcard-font">
									<p v-if="!item.filename.endsWith('/')">
										{{
											item.filesize <= 1024
												? item.filesize + "KB"
												: (item.filesize / 1000).toFixed(1) + "MB"
										}}
									</p>
								</div>
							</el-card>
...
const buttons = ref([
	{
		...
		types: "primary-4th",
		content: "返回",
		cover: "https://f005.backblazeb2.com/file/img-forWeb/uPic/icons8-back-to-256.png",
	},
	{
		index: 2,
		types: "primary-3rd",
		content: "上传",
		cover: "https://f005.backblazeb2.com/file/img-forWeb/uPic/Upload.png",
	},
	{
		index: 3,
		types: "primary-2nd",
		content: "下载",
		cover: "https://f005.backblazeb2.com/file/img-forWeb/uPic/Below.png",
	},
	{
		index: 4,
		types: "primary-3rd",
		content: "删除",
		cover: "https://f005.backblazeb2.com/file/img-forWeb/uPic/Close.png",
	},
	{
		index: 5,
		types: "primary-3rd",
		content: "重命名",
		cover: "https://f005.backblazeb2.com/file/img-forWeb/uPic/Edit.png",
	},
	{
		index: 6,
		types: "primary-3rd",
		content: "分享",
		cover: "https://f005.backblazeb2.com/file/img-forWeb/uPic/Share.png",
	},
	{
		index: 7,
		types: "primary-3rd",
		content: "新建文件夹",
		cover: "https://f005.backblazeb2.com/file/img-forWeb/uPic/Add%20Folder.png",
	},
	{
		index: 8,
		types: "primary-3rd",
		content: "全选",
		cover: "https://f005.backblazeb2.com/file/img-forWeb/uPic/icons8-ok-256.png",
	},
]);
...
const open = (filepath, filename) => {
	if (
		...
		fileType.value = ".mp3";
		URL.value =
			localServer + "/file/getFile?owner=" + localStorage.getItem("id") + "&path=" + filepath;
		ShowDetailDialog.value = true;
...
const remove = () => {
	for (let i = 0; i < checkList.value.length; i++) {
		setTimeout(() => {
			axios.get("/file/remove?id=" + fileList.value[checkList.value[i]].id).then((res) => {
				if (res.data) {
...
const handleButtonClick = (index) => {
	if (index === 0) {
		if (path.value === "/") {
			...
			path.value = path.value.substring(
				0,
				path.value.lastIndexOf("/", path.value.lastIndexOf("/") - 1) + 1,
			);
			search();
		}
	} else if (index === 1) {
		...
		if (checkList.value.length === 0) {
			...
			for (let i = 0; i < checkList.value.length; i++) {
				window.open(
					localServer + "/file/download?path=" + fileList.value[checkList.value[i]].filepath,
				);
				wait(200);
			}
			checkList.value = [];
		...
		if (checkList.value.length != 1) {
			...
			axios.get("/file/share?id=" + fileList.value[0].id).then((res) => {
				if (res.data !== "error") {
